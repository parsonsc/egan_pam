...
?>
    ...
    <ul class="grid news_grid" id="grid">
        ...
        ?>                  
            <li <?php //echo ($hidethis) ? 'style="display:none"':'' ?> class="<?php echo ($full == 1) ? 'one_block':'two_block';?> all <?php $first = ''; $x=0;foreach(get_the_terms( $id, 'casesa' ) as $category) {if ($x < 1) $first=$category->slug;  echo $category->slug . ' ';} ?>" data-category-type="<?php $x=0; foreach(get_the_terms( $id, 'casesa' ) as $category) {if ($x < 1) $first=$category->slug;  echo $category->slug . ' ';};?>">
